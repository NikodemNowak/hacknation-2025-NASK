def generate_simple_pdf(filename="presentation_all_in.pdf", num_pages=5):
    ...
    for i in range(num_pages):
        ...
        pdf_content.append(
            "/Resources << /ProcSet [/PDF /Text] /Font << /F1 << /Type /Font /Subtype /Type1 /Name /F1 /BaseFont /Helvetica /Encoding /MacRomanEncoding >> >> >>"
        )
    ...
    pdf_content[pdf_content.index(f"/Kids [] >>")] = (
        f"/Kids [{kids_array_str}] >>"
    )
    ...
    xref_table.append(
        f"0 {current_obj_id}"
    )  # 0 for free object list, then total objects
    ...
    pdf_content.append(
        "<< /Size {} /Root {} >>".format(current_obj_id, obj_catalog_ref)
    )
