"""
Biblioteka do anonimizacji danych osobowych w tekście za pomocą wyrażeń regularnych.
...
class AnonymizationResult:
    ...
    original_text: str
    anonymized_text: str
    replacements: List[Tuple[str, str, str]]  # (original, tag, position)
...
class RegexAnonymizer:
    ...
    def _compile_patterns(self):
        ...
        # Wzorce dla PESEL z możliwymi literami zamiast cyfr (np. '0' -> 'O', '1' -> 'l')
        self.pesel_pattern = re.compile(
            r'\b'  # granica słowa
            r'(?:'
                # Standardowy PESEL: 11 cyfr
                r'[0-9oOlI!|]{11}'
            r')'
            r'\b',
            re.IGNORECASE
        ...
        self.email_pattern = re.compile(
            r'[a-zA-Z0-9._%+\-łśżźćńóęą|!]{1,64}'  # część lokalna, może zawierać polskie znaki i zniekształcenia
            r'@'
            r'[a-zA-Z0-9.\-łśżźćńóęą|]{1,255}'  # domena
            r'\.'
            r'[a-zA-Z]{2,10}',  # TLD
            re.IGNORECASE
        )
        ...
        # Formaty: +48 XXX XXX XXX, 48 XXX XXX XXX, XXX XXX XXX, XXX-XXX-XXX, itp.
        # Obsługuje zniekształcenia: B->8, S->5, O->0, I/l->1, A->4, E->3, G->9, q->9, h->4, z->2
        self.phone_pattern = re.compile(
            r'(?:'
                # Format z +48 lub 48 na początku
                r'(?:\+\s*[4hA]\s*[8B]\s*|\b[4hA][8B]\s+)?'
                r'(?:'
                    # Format z spacjami/myślnikami: XXX XXX XXX lub XX XXX XX XX
        ...
        # Numer konta bankowego (IBAN polski) - 26 cyfr lub z PL na początku
        # Format: PL XX XXXX XXXX XXXX XXXX XXXX XXXX lub same cyfry
        self.bank_account_pattern = re.compile(
            r'\b'
            r'(?:'
                # Z prefiksem PL
                r'(?:PL\s*)?'
                # 26 cyfr z możliwymi spacjami
        ...
        # Karta kredytowa - 16 cyfr (może być w grupach po 4)
        # Format: XXXX XXXX XXXX XXXX lub XXXX-XXXX-XXXX-XXXX
        ...
        # Format: 3 litery + 6 cyfr lub różne warianty z separatorami
        # Np: ABC123456, ABC 123456, ABC-123456
        # Lub format: XXXX-XXXX-XXXX (jak w danych)
        self.document_number_pattern = re.compile(
            r'\b'
            r'(?:'
                # Klasyczny format: 3 litery + 6 cyfr
                r'[A-Za-z]{2,3}\s*[0-9oOlI!|]{4,6}'
                r'|'
                # Format XXXX-XXXX-XXXX (z cyfr i separatorów)
    ...
    def _is_valid_pesel(self, text: str) -> bool:
        ...
        Konwertuje typowe zniekształcenia liter na cyfry.
        """
        # Zamień typowe zniekształcenia
        cleaned = text.upper()
        ...
        # Podstawowa walidacja PESEL (może być syntetyczny w danych testowych)
        # Sprawdzamy tylko format, nie sumę kontrolną
        return True
    
    def _is_valid_phone(self, text: str) -> bool:
        """
        Sprawdza czy tekst może być numerem telefonu.
        Obsługuje różne zniekształcenia znaków.
        """
        # Wyczyść z separatorów
    ...
    def _is_valid_email(self, text: str) -> bool:
        ...
        # Email musi mieć @ i domenę z kropką
    ...
    def _is_valid_document_number(self, text: str) -> bool:
        ...
        # Format klasyczny: 3 litery + 6 cyfr (lub 2 litery + 7 cyfr)
        if re.match(r'^[A-Z]{2,3}\d{4,7}$', cleaned):
            return True
        
        # Format XXXX-XXXX-XXXX (12 cyfr z separatorami)
    ...
    def anonymize(self, text: str) -> str:
        ...
        # 1. Email - najbardziej charakterystyczny
        ...
        # 6. Telefon (na końcu, bo może się pokrywać z innymi)
    ...
    def anonymize_detailed(self, text: str) -> AnonymizationResult:
        ...
        # Znajdź wszystkie zamiany
        replacements = []
        
        # Email
        for match in self.email_pattern.finditer(text):
            if self._is_valid_email(match.group(0)):
                replacements.append((match.group(0), 'email', str(match.start())))
        
        # PESEL
        for match in self.pesel_pattern.finditer(text):
            if self._is_valid_pesel(match.group(0)):
                replacements.append((match.group(0), 'pesel', str(match.start())))
        
        # Telefon
        for match in self.phone_pattern.finditer(text):
            if self._is_valid_phone(match.group(0)):
                replacements.append((match.group(0), 'phone', str(match.start())))
...
def anonymize_text(text: str, use_brackets: bool = True) -> str:
    """
...
if __name__ == "__main__":
    # Przykładowy tekst
    example = """
    Hej, jestem Jan Kowalski. Mój PESEL to 90010112345, 
    a email jan.kowalski@example.com. Zadzwoń na +48 123 456 789.
    Numer dowodu: ABC123456.
    """
    
    anonymizer = RegexAnonymizer(use_brackets=True)
    result = anonymizer.anonymize(example)
