...
def highlight_differences(
    ...
    return {
        ...
        'match_ratio': (
            our_found / max(regex_tags_in_ref, 1)
            if regex_tags_in_ref > 0
            else 1.0
        ),
    }
...
def demo_single_line(
    ...
    print(
        ...
        + ("..." if len(result.anonymized_text) > 500 else "")
    )
...
def main():
    ...
    for idx, original, reference in pairs:
        ...
        if (
            stats['our_tags_used'] > 0
            and stats['our_tags_used'] >= stats['regex_tags_in_ref']
        ):
            matches += 1
    ...
    print(
        f"   • Łączna liczba tagów regex w referencji: {total_ref_regex_tags}"
    )
    print(
    ...
    if good_examples:
        print()
        print(
            f"🎯 PRZYKŁAD Z WIELOMA WYKRYTYMI DANYMI (znaleziono {good_examples[0]['our_tags_used']} tagów):"
        )
        print("-" * 60)
        print("Oryginał (fragment):")
